...
function applyRoleUI(){
  ...
  // Sell price column — admin only
  const spHdr=document.getElementById('sell-price-col-hdr'); if(spHdr) spHdr.style.display=isAdmin?'':'none';
  ...
  // Revenue incl. GST and Profit/Loss cards — admin only
  const revenueInclCard=document.querySelector('.stat-card.s-revenue');
  if(revenueInclCard) revenueInclCard.style.display=isAdmin?'flex':'none';
  const profitCard=document.getElementById('profit-card');
  if(profitCard) profitCard.style.display=isAdmin?'flex':'none';
}
...
function renderRecentDispatches(){
  ...
  el.innerHTML=`<div class="tbl-wrap"><table class="rd-table dtable">
    <thead><tr>
...
function renderTable(){
  ...
  tbody.innerHTML=rows.map(([id,i])=>{
    ...
    return `<tr${isLow?' class="low-stock"':''}>
      ...
      <td style="font-family:var(--fmono);font-size:.8rem;color:var(--green);font-weight:600"${isAdmin?'':' hidden'}>${(parseFloat(i.sellPrice)||0)>0?'₹'+fmt(i.sellPrice):'—'}</td>
